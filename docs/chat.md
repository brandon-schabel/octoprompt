...
- Toggle the **"Exclude" switch** to remove this message (and subsequent messages if it's an assistant response that depends on it) from the context sent to the AI for future turns in _this_ chat. This can be useful if a message is irrelevant or misleading for the ongoing conversation.
