...
Welcome to the Provider Keys Page! This section of Promptliano is crucial for unlocking the full suite of AI-powered features. Here, you'll manage the API keys that connect Promptliano to various external and local AI model providers.
...
1. [Understanding Provider Keys](#understanding-provider-keys)
   ...
   - [Why Does Promptliano Need Them?](#why-does-promptliano-need-them)
...
API (Application Programming Interface) keys are unique codes that applications use to authenticate and authorize requests to external services. In the context of Promptliano, these keys allow the application to securely communicate with AI model providers. Think of them as a password that Promptliano uses to tell an AI provider that it has permission to use its services on your behalf.

### Why Does Promptliano Need Them?

Promptliano leverages powerful Large Language Models (LLMs) from various providers to offer advanced features such as:
...
- **Built-in AI Chat:** Interact with different LLMs directly within Promptliano.

To use these external AI services, Promptliano needs the respective API keys you provide. Without them, AI features that rely on these providers won't be available.
...
- **Local Providers:** Services like Ollama or LM Studio allow you to run AI models directly on your own machine. For these, you might not need an "API key" in the traditional sense to be pasted into Promptliano, but you'll need to have the local service running and configured. Promptliano can then interact with them, often without any external an API key entry on this page.

---

## 2. Accessing the Provider Keys Page

You can typically find the Provider Keys page via a dedicated "Keys" or "API Keys" link in the main navigation menu or settings area of Promptliano.
...
- **Dropdown Menu:** A selectable list of all AI providers that Promptliano supports. You'll choose a provider from this list before adding or modifying its key.
...
- **Add Button:** After selecting a provider and entering its key, click this button to save the key to Promptliano. This button will be disabled if the necessary information (provider and key, if required) isn't provided.
...
Promptliano supports a range of AI providers to give you flexibility. As of the latest information, these include:
...
_(This list may be updated in newer versions of Promptliano. Always refer to the provider selection dropdown on the page for the most current list.)_
...
6. Return to the Promptliano Provider Keys page to add it.
...
1. Navigate to the **Provider Keys** page in Promptliano.
...
If you need to copy the full API key you've previously saved (for example, to use it in another application or for verification), Promptliano provides a convenient way to do so without re-typing it.
...
If you no longer want Promptliano to use a specific API key, or if a key has been compromised, you can delete it.
...
4. The key will be removed from the list and Promptliano will no longer use it.
...
- **No API Key Entry in Promptliano:** You typically don't need to enter an API key on this Promptliano "Provider Keys" page for these local providers. The placeholder text in the API key field will usually indicate this (e.g., "No API Key needed, but might need local config").
...
- **Promptliano Configuration:** Promptliano will then connect to these local services, often at a default local address (e.g., `http://localhost:11434` for Ollama). You might need to configure the specific model to use within Promptliano's chat settings or other relevant sections.

Refer to the Promptliano documentation on Chat or specific AI features, as well as the documentation for Ollama or LM Studio, for detailed setup instructions.
...
- **Security:** Treat your API keys like passwords. Do not share them publicly or commit them to version control (e.g., in a public GitHub repository). Promptliano automatically encrypts your API keys using AES-256-GCM encryption before storing them locally on your machine.
