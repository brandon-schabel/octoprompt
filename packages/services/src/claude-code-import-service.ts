import { claudeCodeStorage } from '@promptliano/storage'
import { chatStorage } from '@promptliano/storage'
import { chatMessageStorage } from '@promptliano/storage'
...
export class ClaudeCodeImportService {
  ...
  async importSession(projectId: number, sessionId: string): Promise<Chat> {
    ...
    const messages = await claudeCodeStorage.getMessages(projectId, sessionId)
    ...
    const importPromises = messages.map((message, index) => 
      this.importMessage(chat.id, message, index)
    )
  ...
  private generateChatTitle(firstUserMessage: ClaudeMessage | undefined, sessionId: string): string {
    ...
    return `Claude Code Session ${sessionId.substring(0, 8)}`
  }
  ...
  private async importMessage(chatId: number, claudeMessage: ClaudeMessage, order: number): Promise<ChatMessage> {
    ...
    return chatMessageStorage.createMessage({
      chatId,
      role,
      content,
      created: timestamp,
   ...
   */
  private extractTextContent(content: string | ClaudeContentItem[]): string {
