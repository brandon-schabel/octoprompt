...
export function validateAIResponse(response: string, expectedElements: string[] = []): ValidationResult {
  ...
  if (response.length < LOCAL_MODEL_TEST_CONFIG.quality.minSummaryLength) {
    errors.push(`Response too short: ${response.length} chars (min: ${LOCAL_MODEL_TEST_CONFIG.quality.minSummaryLength})`)
  }
  
  // Check maximum length
  if (response.length > LOCAL_MODEL_TEST_CONFIG.quality.maxSummaryLength) {
    warnings.push(`Response too long: ${response.length} chars (max: ${LOCAL_MODEL_TEST_CONFIG.quality.maxSummaryLength})`)
  }
  
  // Check for required elements
  const requiredElements = expectedElements.length > 0 ? expectedElements : LOCAL_MODEL_TEST_CONFIG.quality.requiredElements
...
export class PerformanceTracker {
  ...
  getMetrics(tokensUsed: number, cacheHit: boolean = false): PerformanceMetrics {
    ...
    return {
      ...
      memoryUsed: this.endMemory && this.startMemory 
...
function calculateRelevanceScore(summary: string, sourceCode: string): number {
  ...
  const uniqueIdentifiers = [...new Set(identifiers.map(id => {
  ...
  const mentionedCount = uniqueIdentifiers.filter(id => 
    summaryLower.includes(id)
  ).length
...
export async function retryWithBackoff<T>(
  fn: () => Promise<T>,
  options = LOCAL_MODEL_TEST_CONFIG.retry
): Promise<T> {
