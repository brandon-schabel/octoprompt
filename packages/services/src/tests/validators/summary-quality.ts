...
export class SummaryQualityValidator {
  ...
  private evaluateLength(summary: string, minLength: number, maxLength: number, issues: SummaryIssue[]): number {
  ...
  private evaluateTokenEfficiency(summary: string, sourceCode: string): number {
    ...
    if (ratio > 0.3) return 60 // Too verbose
    if (ratio >= 0.05 && ratio <= 0.2) return 100 // Optimal
