...
export class MetricsCollector {
    ...
    return Effect.gen(
      function* (_) {
        ...
        this.aggregationSchedule = yield* _(
          Effect.repeat(this.aggregateMetrics(), Schedule.fixed(Duration.millis(this.aggregationInterval))).pipe(
   ...
   */
  record(metric: string, value: number, tags?: Record<string, string>): Effect.Effect<void, never> {
  ...
  recordOptimization(
    ...
    return Effect.gen(
      function* (_) {
        ...
        yield* _(this.record(`optimization.confidence.${optimizer}`, optimized.optimizationStrategy.confidence))
      }.bind(this)
    )
  }
   ...
   */
  getTimeSeries(metric: string, start?: number, end?: number): Effect.Effect<TimeSeriesData | null, never> {
    return Effect.gen(
      function* (_) {
        const metricsMap = yield* _(Ref.get(this.metrics))
        const values = HashMap.get(metricsMap, metric)
        ...
        const filteredValues = values.value.filter((v) => v.timestamp >= startTime && v.timestamp <= endTime)
  ...
  getOptimizationReport(optimizer?: string): Effect.Effect<OptimizationMetrics, never> {
    return Effect.gen(
      function* (_) {
        ...
        const totalOptimizations = Object.values(durationMetrics).reduce((sum, m) => sum + m.count, 0)

        const avgDuration =
          Object.values(durationMetrics).reduce((sum, m) => sum + m.mean, 0) /
          Math.max(1, Object.keys(durationMetrics).length)

        const throughput =
          totalOptimizations > 0 ? (totalOptimizations / (Date.now() - this.getOldestTimestamp())) * 1000 : 0
  ...
  private aggregateMetrics(): Effect.Effect<void, never> {
    return Ref.update(this.metrics, (map) => {
      ...
      return HashMap.map(map, (values) => values.filter((v) => v.timestamp > cutoff))
    })
  }
  ...
  private filterMetrics(metrics: Record<string, MetricSummary>, pattern: string): Record<string, MetricSummary> {
    const result: Record<string, MetricSummary> = {}
  ...
  private getPercentileFromMetrics(metrics: Record<string, MetricSummary>, percentile: keyof MetricSummary): number {
 ...
 * Record a metric value
 */
export function recordMetric(metric: string, value: number, tags?: Record<string, string>): Effect.Effect<void, never> {
 ...
 * Get optimization report
 */
export function getOptimizationReport(optimizer?: string): Effect.Effect<OptimizationMetrics, never> {
