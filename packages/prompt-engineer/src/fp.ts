...
export const TE = {
    tryCatch: <E, A>(thunk: () => Promise<A>, onError: (e: unknown) => E): TaskEither<E, A> =>
        async () => {
            try { return E.right(await thunk()) } catch (err) { return E.left(onError(err)) }
        },
    fromEither: <E, A>(ea: Either<E, A>): TaskEither<E, A> => async () => ea,
...
export const A = {
    mapWithIndex: <A, B>(f: (i: number, a: A) => B) => (as: A[]): B[] => as.map((a, i) => f(i, a))
}

export const O = {
    fromPredicate: <A>(pred: (a: A) => boolean) => (a: A) => (pred(a) ? { _tag: 'Some', value: a } : { _tag: 'None' as const }),
