...
export class IndexedDBStoragePlugin implements StoragePlugin {
  ...
  initialize(): Effect.Effect<void, StorageErrorClass> {
    return Effect.gen(function* (_) {
      ...
      if (typeof window === 'undefined' || !window.indexedDB) {
        return yield* _(Effect.fail(new StorageErrorClass({
  ...
  get<T>(key: string): Effect.Effect<CacheEntry<T> | null, StorageErrorClass> {
    return Effect.gen(function* (_) {
      ...
      const result = yield* _(Effect.tryPromise({
        try: () => new Promise<CacheEntry<T> | null>((resolve, reject) => {
          ...
          request.onsuccess = () => {
            ...
            if (entry.ttl && entry.ttl > 0) {
              ...
              if (age > entry.ttl) {
                ...
                this.delete(key).pipe(Effect.runPromise).catch(() => {})
                resolve(null)
                return
              }
            }
        ...
        catch: (error) => new StorageErrorClass({
          operation: 'read' as const,
          key,
  ...
  has(key: string): Effect.Effect<boolean, StorageErrorClass> {
    return Effect.gen(function* (_) {
      ...
      const exists = yield* _(Effect.tryPromise({
        try: () => new Promise<boolean>((resolve, reject) => {
          const transaction = db.transaction([this.config.storeName!], 'readonly')
          const store = transaction.objectStore(this.config.storeName!)
  ...
  keys(): Effect.Effect<readonly string[], StorageErrorClass> {
    return Effect.gen(function* (_) {
      ...
      const allKeys = yield* _(Effect.tryPromise({
  ...
  searchByIndex<T>(
    ...
    return Effect.gen(function* (_) {
      ...
      const results = yield* _(Effect.tryPromise({
        try: () => new Promise<Array<CacheEntry<T>>>((resolve, reject) => {
          const transaction = db.transaction([this.config.storeName!], 'readonly')
          const store = transaction.objectStore(this.config.storeName!)
  ...
  private openDatabase(): Effect.Effect<IDBDatabase, StorageErrorClass> {
    return Effect.tryPromise({
      try: () => new Promise<IDBDatabase>((resolve, reject) => {
  ...
  private getDatabase(): Effect.Effect<IDBDatabase, StorageErrorClass> {
    if (!this.db) {
      return Effect.fail(new StorageErrorClass({
        operation: 'read' as const,
