...
import { PluginManagerTag, PluginRegistryTag } from './types'
import { createPluginManager, createPluginRegistry, PluginManagerLive, PluginRegistryLive } from './registry'
...
export class PluginSystem {
   ...
   */
  async initialize(plugins?: (string | PromptEngineerPlugin)[]): Promise<void> {
   ...
   */
  async getPlugin<T extends PromptEngineerPlugin>(name: string): Promise<T | null> {
    if (!this.manager) {
      throw new Error('Plugin system not initialized')
    }
  ...
  async getProvider(name?: string): Promise<ProviderPlugin | null> {
    ...
    const providers = await Effect.runPromise(this.manager.getPlugins<ProviderPlugin>('provider'))
  ...
  async getStorage(name?: string): Promise<StoragePlugin | null> {
    ...
    const storages = await Effect.runPromise(this.manager.getPlugins<StoragePlugin>('storage'))
  ...
  async getMonitoring(): Promise<MonitoringPlugin | null> {
    ...
    const monitors = await Effect.runPromise(this.manager.getPlugins<MonitoringPlugin>('monitoring'))
 ...
 */
export async function initializePluginSystem(plugins?: (string | PromptEngineerPlugin)[]): Promise<PluginSystem> {
 ...
 */
export function createPlugin(name: string, version: string, capabilities: string[] = []): PromptEngineerPlugin {
  return {
    name,
    version,
    capabilities,
 ...
 */
export function isProviderPlugin(plugin: PromptEngineerPlugin): plugin is ProviderPlugin {
 ...
 */
export function isStoragePlugin(plugin: PromptEngineerPlugin): plugin is StoragePlugin {
 ...
 */
export function isDatasetPlugin(plugin: PromptEngineerPlugin): plugin is DatasetPlugin {
 ...
 */
export function isMonitoringPlugin(plugin: PromptEngineerPlugin): plugin is MonitoringPlugin {
...
export { PluginManagerLive, PluginRegistryLive, PluginManagerTag, PluginRegistryTag }
