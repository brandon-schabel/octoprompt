...
export class MultiModalManager {
  ...
  processMultiModal(context: MultiModalContext): Effect.Effect<CombinedAnalysis, Error> {
    return Effect.gen(
      function* (_) {
        ...
        const results = yield* _(Effect.all(context.media.map((file) => this.processMedia(file))))
   ...
   */
  queryMultiModal(media: MediaFile[], query: string): Effect.Effect<string, Error> {
    return Effect.gen(
      function* (_) {
        ...
        const responses = yield* _(
          Effect.all(analysis.media.map((result, index) => this.queryMediaResult(result, query, index)))
        )
  ...
  > {
    return Effect.gen(
      function* (_) {
        const [result1, result2] = yield* _(Effect.all([this.processMedia(media1), this.processMedia(media2)]))

        // Type-specific comparison
        if (result1.type === result2.type) {
          switch (result1.type) {
            case 'image':
              const imageCmp = yield* _(this.imageAdapter.compareImages(media1.data, media2.data))
              return {
            ...
            case 'audio':
              const audioCmp = yield* _(this.audioAdapter.compareAudio(media1.data, media2.data))
              return {
            ...
            case 'document':
              const docCmp = yield* _(this.documentAdapter.compareDocuments(media1.data, media2.data))
              return {
        ...
        const textSimilarity = this.compareText(result1.text || '', result2.text || '')
  ...
  extractAllText(media: MediaFile[]): Effect.Effect<string, Error> {
    return Effect.gen(
      function* (_) {
        const results = yield* _(Effect.all(media.map((file) => this.processMedia(file))))
  ...
  private analyzeRelationships(results: MultiModalResult[]): Effect.Effect<MediaRelationship[], never> {
    return Effect.gen(function* (_) {
      ...
      for (let i = 0; i < results.length; i++) {
        for (let j = i + 1; j < results.length; j++) {
          const similarity = this.compareText(results[i].text || '', results[j].text || '')
  ...
  private generateInsights(results: MultiModalResult[], relationships: MediaRelationship[]): string[] {
    ...
    if (images.length > 0) {
      const avgQuality =
        images.reduce((acc, r) => acc + ((r.analysis as any).quality?.aestheticScore || 0), 0) / images.length
      insights.push(`Average image quality: ${(avgQuality * 100).toFixed(1)}%`)
    }

    const audio = results.filter((r) => r.type === 'audio')
    if (audio.length > 0) {
      const totalDuration = audio.reduce((acc, r) => acc + ((r.analysis as any).metadata?.duration || 0), 0)
      insights.push(`Total audio duration: ${totalDuration}s`)
    }

    const documents = results.filter((r) => r.type === 'document')
    if (documents.length > 0) {
      const totalPages = documents.reduce((acc, r) => acc + ((r.analysis as any).pages?.length || 0), 0)
  ...
  private createCombinedSummary(results: MultiModalResult[], prompt: string): string {
  ...
  private queryMediaResult(result: MultiModalResult, query: string, index: number): Effect.Effect<string, never> {
    return Effect.succeed(`Media ${index + 1} (${result.type}): ${result.text?.substring(0, 200) || result.caption}`)
  }
 ...
 */
export function createMultiModalManager(
  config?: ConstructorParameters<typeof MultiModalManager>[0]
): MultiModalManager {
