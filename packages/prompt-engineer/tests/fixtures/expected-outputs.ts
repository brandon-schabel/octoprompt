...
export const validators = {
  ...
  validateSCoTOutput(output: OptimizedPrompt): boolean {
    ...
    const hasSequenceSection = output.userPrompt.includes('SEQUENCE') || 
                               output.userPrompt.includes('STEP')
  ...
  validateContextOutput(output: OptimizedPrompt): boolean {
    ...
    const hasCompression = output.optimizationStrategy.techniques.includes('compression') ||
...
export class OutputValidator {
  ...
  static assertImprovementInRange(
    score: number,
    optimizer: string,
    tolerance: number = 5
  ): void {
