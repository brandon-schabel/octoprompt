...
describe('Plugin System Integration', () => {
  ...
  describe('Storage Plugins', () => {
    ...
    it('should handle encryption in file storage', async () => {
      ...
      if (encryptedFile) {
        const rawContent = await fs.readFile(path.join(testCacheDir, encryptedFile), 'utf-8')
  ...
  describe('Provider Plugins', () => {
    it('should initialize and use mock provider', async () => {
      ...
      // Test generation
      const result = await Effect.runPromise(provider.generate('Optimize this prompt', { temperature: 0.5 }))
    ...
    it('should handle structured generation', async () => {
      ...
      const structured = await Effect.runPromise(provider.generateStructured('Generate structured data', ResultSchema))
    ...
    it('should handle provider errors', async () => {
      ...
      const result = await Effect.runPromise(
        provider.generate('Test').pipe(Effect.catchAll((error) => Effect.succeed({ error: error.message })))
      )
  ...
  describe('Monitoring Integration', () => {
    ...
    it('should track cache operations', async () => {
      ...
      await Effect.runPromise(monitoring.monitorCache('set', 'cache-key', 'memory', storage.set('cache-key', 'value')))

      // Cache hit
      await Effect.runPromise(monitoring.monitorCache('get', 'cache-key', 'memory', storage.get('cache-key')))

      // Cache miss
      await Effect.runPromise(monitoring.monitorCache('get', 'missing-key', 'memory', storage.get('missing-key')))

      const metrics = await Effect.runPromise(monitoring.getMonitoringService().getMetricsSnapshot())
...
describe('Plugin Error Handling', () => {
  it('should handle storage plugin initialization errors gracefully', async () => {
    ...
    const result = await Effect.runPromise(
      storage.initialize().pipe(Effect.catchAll((error) => Effect.succeed({ error: error.message })))
    )
