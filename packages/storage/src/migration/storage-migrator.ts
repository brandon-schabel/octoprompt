  ...
  static createIndexes(indexes: Array<{ name: string; fields: string[] }>): MigrationStep {
    return {
      ...
      execute: async (context) => {
        ...
        // Indexing has been removed from the storage system
        for (const index of indexes) {
          context.log(`Skipping index creation: ${index.name} (indexing feature removed)`)
        }
...
export class StorageMigrationUtils {
  ...
  static async migrateFromLegacyStorage(
    ...
    try {
      ...
      migrator.addStep(
        MigrationSteps.validateData((key, data) => {
   ...
   * Restore from backup
   */
  static async restoreFromBackup(backupPath: string, targetAdapter: StorageAdapter): Promise<MigrationResult> {
    const startTime = Date.now()
    const result: MigrationResult = {
      success: false,
      migratedKeys: 0,
...
export async function migrateStorage(
  ...
  return StorageMigrationUtils.copyStorage(from.type, from.config, to.type, to.config, options)
}
