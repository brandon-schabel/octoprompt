...
# Last 5 changes:
# 1. Fixed timestamp field names (created/updated instead of created_at/updated_at).
# 2. Fixed timestamp values to use integer Unix milliseconds.
# 3. Fixed storage access to use direct dict access instead of .data.
# 4. Fixed ID types to use int consistently.
# 5. Completed missing method implementations.
# 6. Added model_config handling with default LOW_MODEL_CONFIG.

class ChatService:
    async def _update_chat_timestamp(self, chat_id: int) -> None:
        all_chats = await chat_storage.read_chats()
        chat_entry = all_chats.get(chat_id)  # Direct dict access, no .data
        if not chat_entry:
            raise ApiError(
                status=404, 
                message=f"Chat with ID {chat_id} not found for timestamp update.", 
                code='CHAT_NOT_FOUND'
            )
        
        # Update with int timestamp
    ...
    async def create_chat(self, title: str, copy_existing: Optional[bool] = None, current_chat_id: Optional[int] = None, model_config: Optional[ModelOptions] = None) -> Chat:
        ...
        # Use provided model_config or default to LOW_MODEL_CONFIG
        ...
        if copy_existing and current_chat_id:
            # current_chat_id is already int due to type hint
            if current_chat_id not in all_chats:
                raise ApiError(
                    status=404, 
                    message=f"Referenced chat {current_chat_id} not found.", 
                    code='REFERENCED_CHAT_NOT_FOUND'
                )
        
        if chat_id in all_chats:
            raise ApiError(
                status=409, 
                message=f"Chat ID conflict for {chat_id}", 
                code='CHAT_ID_CONFLICT'
            )

        all_chats[chat_id] = new_chat_data
        await chat_storage.write_chats(all_chats)
        await chat_storage.write_chat_messages(chat_id, {})  # Initialize empty messages

        if copy_existing and current_chat_id:
            source_chat_id = current_chat_id # Already an int
            ...
            for msg_id, msg_data in source_messages.items():
                ...
                copied_msg = msg_data.model_copy(update={
                    "id": new_msg_id, 
                    "chatId": chat_id
                })
    ...
    async def save_message(self, message: ExtendedChatMessage) -> ExtendedChatMessage:
        ...
        if chat_id not in all_chats:
            raise ApiError(
                ...
                code='CHAT_NOT_FOUND_FOR_MESSAGE'
            )
        ...
        final_message_data = ChatMessage(
            ...
            created=message.created or now_ms
        )

        chat_messages = await chat_storage.read_chat_messages(chat_id)
        
        if message_id in chat_messages and message.id:
            print(f"Warning: Message with ID {message_id} already exists in chat {chat_id}. Overwriting.")
        ...
        return ExtendedChatMessage(
            **final_message_data.model_dump(), 
    ...
    async def get_all_chats(self) -> List[Chat]:
        ...
        chat_list.sort(key=lambda c: c.updated, reverse=True)  # Use 'updated'
        return chat_list
    ...
    async def get_chat_messages(self, chat_id: int) -> List[ChatMessage]:
        ...
        message_list.sort(key=lambda m: m.created)  # Use 'created'
        return message_list
    ...
    async def update_chat(self, chat_id: int, title: str) -> Chat:
        ...
        if chat_id not in all_chats:
            raise ApiError(
                status=404, 
                message=f"Chat {chat_id} not found for update.", 
                code='CHAT_NOT_FOUND'
            )
        
        now_ms = int(datetime.now(timezone.utc).timestamp() * 1000)
        updated_chat = all_chats[chat_id].model_copy(update={
            "title": title,
            "updated": now_ms
        })
    ...
    async def delete_chat(self, chat_id: int) -> None:
        ...
        if chat_id not in all_chats:
            raise ApiError(
                status=404, 
                message=f"Chat {chat_id} not found for deletion.", 
                code='CHAT_NOT_FOUND'
            )
    ...
    async def delete_message(self, chat_id: int, message_id: int) -> None:
        ...
        if message_id not in messages:
            raise ApiError(
                status=404, 
                message=f"Message {message_id} not found in chat {chat_id}.", 
    ...
    async def fork_chat(self, source_chat_id: int, excluded_message_ids: List[int] = None) -> Chat:
        ...
        if source_chat_id not in all_chats:
            raise ApiError(
                status=404,
                message=f"Source chat {source_chat_id} not found.",
                code='SOURCE_CHAT_NOT_FOUND'
            )
        
        source_chat = all_chats[source_chat_id]
        
        # Create new chat with title indicating it's a fork
    ...
    async def fork_chat_from_message(
        ...
        source_chat_id: int, 
        message_id: int, 
        excluded_message_ids: List[int] = None
    ) -> Chat:
        excluded_message_ids = excluded_message_ids or []
        ...
        if message_id not in source_messages:
            raise ApiError(
                status=404,
                message=f"Message {message_id} not found in chat {source_chat_id}.",
                code='MESSAGE_NOT_FOUND'
            )
        ...
        for msg_id, msg_data in source_messages.items():
            if (msg_data.created <= target_timestamp and 
                msg_id not in excluded_message_ids):
                new_msg_id = chat_storage.generate_id()
                copied_msg = msg_data.model_copy(update={
                    "id": new_msg_id,
                    "chatId": new_chat.id
