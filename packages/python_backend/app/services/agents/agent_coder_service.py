...
from app.services.project_service import bulk_create_project_files, FileSyncData
...
async def run_planning_agent(agent_context: PyCoderAgentDataContext) -> PyTaskPlan:
    ...
    try:
        response = await generate_structured_data(
            ...
            options={**PLANNING_MODEL_CONFIG, "temperature": PLANNING_MODEL_CONFIG.get("temperature", 0.7)},
            system_message=planning_system_prompt,
        )
        result_obj = response.object 
...
async def run_file_rewrite_agent(
    ...
    try:
        ...
        response = await generate_structured_data(
            ...
            schema=agent_coder_prompts["file_rewrite_agent"]["schema"], 
...
async def main_orchestrator(raw_agent_context_input: PyCoderAgentDataContext) -> CoderAgentOrchestratorSuccessResult:
    ...
    try:
        ...
        print(f"[Orchestrator CRITICAL] Logger initialization failed for {agent_job_id}: {str(init_error)}") 
