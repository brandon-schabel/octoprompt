...
async def create_prompt(data: CreatePromptBody) -> Prompt:
    prompt_id = prompt_storage_util.generate_id("prompt")
    ...
    if data.project_id:
        await add_prompt_to_project(new_prompt.id, data.project_id)

    return new_prompt

async def add_prompt_to_project(prompt_id: str, project_id: str) -> None:
    all_prompts = await prompt_storage_util.read_prompts()
    ...
    new_link_data = {
        "id": prompt_storage_util.generate_id("link"),
        "prompt_id": prompt_id,
...
async def remove_prompt_from_project(prompt_id: str, project_id: str) -> None:
    prompt_projects = await prompt_storage_util.read_prompt_projects()
...
async def get_prompt_by_id(prompt_id: str) -> Prompt:
    all_prompts = await prompt_storage_util.read_prompts()
...
async def get_prompts_by_ids(prompt_ids: List[str]) -> List[Prompt]:
    all_prompts = await prompt_storage_util.read_prompts()
    return [all_prompts[id_] for id_ in prompt_ids if id_ in all_prompts]

async def list_prompts_by_project(project_id: str) -> List[Prompt]:
    prompt_projects = await prompt_storage_util.read_prompt_projects()
...
async def update_prompt(prompt_id: str, data: UpdatePromptBody) -> Prompt:
    all_prompts = await prompt_storage_util.read_prompts()
    ...
    updated_prompt_data.updated_at = datetime.now(timezone.utc)
...
async def get_prompt_projects(prompt_id: str) -> List[PromptProject]:
    prompt_projects = await prompt_storage_util.read_prompt_projects()
