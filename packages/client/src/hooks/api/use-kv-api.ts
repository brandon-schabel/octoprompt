...
export function useSetKvValue<K extends KVKey>(key: K) {
    ...
    return useMutation<
        ...
        MutationContext<K>
    >({
        mutationFn: async (payload: SpecificValueType) => { // Explicitly type payload
...
export function useActiveProjectTab(): [ProjectTabState, (partialData: Partial<ProjectTabState>) => void, string | null] {
    const { activeProjectTabId } = useGetActiveProjectTabId()
