...
import type { Job, JobStatus } from '@octoprompt/schemas'
...
export function JobMonitor({ projectId, className }: JobMonitorProps) {
  ...
  const jobsByStatus = jobs.reduce((acc: Record<JobStatus, number>, job: Job) => {
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Monitor and manage background operations for this project
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              {jobsByStatus.failed > 0 && (
                <Badge variant="destructive">
                  {jobsByStatus.failed} Failed
                </Badge>
              )}
              {jobsByStatus.cancelled > 0 && (
                <Badge variant="outline">
                  {jobsByStatus.cancelled} Cancelled
                </Badge>
              )}
            </div>
