...
import type { Job } from '@promptliano/schemas'
          ...
          <div className='space-y-1'>
            ...
            <CardDescription className='text-xs'>
              <div className='flex items-center gap-4'>
                ...
                {job.metadata?.path && <span className='font-mono text-xs'>{job.metadata.path}</span>}
              </div>
            </CardDescription>
          </div>
          <div className='flex items-center gap-1'>
            {canCancel && onCancel && (
              <Button size='sm' variant='ghost' onClick={() => onCancel(job.id)} title='Cancel job'>
                <X className='h-3 w-3' />
              </Button>
            )}
            {canRetry && onRetry && (
              <Button size='sm' variant='ghost' onClick={() => onRetry(job.id)} title='Retry job'>
      ...
      <CardContent className='pt-0'>
        {job.progress && (
          <div className='space-y-2'>
            <div className='flex items-center justify-between text-xs text-muted-foreground'>
              ...
              {job.progress.percentage !== undefined && <span>{Math.round(job.progress.percentage)}%</span>}
            </div>
        ...
        {job.status === 'completed' && job.result && (
          <div className='mt-2 text-xs text-muted-foreground'>
            ...
            {job.type.includes('add') && job.result.path && <span>Created at: {job.result.path}</span>}
            {job.type.includes('remove') && job.result.path && <span>Removed: {job.result.path}</span>}
          </div>
        )}
      </CardContent>
    </Card>
