...
import type { TokenUsage } from '@promptliano/schemas'
...
export function TokenUsageTooltip({ tokenUsage, children, className, showTotal = true }: TokenUsageTooltipProps) {
  ...
  const input =
    'input_tokens' in tokenUsage
      ...
      : 'totalInputTokens' in tokenUsage
        ? tokenUsage.totalInputTokens
        : undefined
  const cacheCreation =
    'cache_creation_input_tokens' in tokenUsage
      ? tokenUsage.cache_creation_input_tokens
      : 'totalCacheCreationTokens' in tokenUsage
        ? tokenUsage.totalCacheCreationTokens
        : undefined
  const cacheRead =
    'cache_read_input_tokens' in tokenUsage
      ? tokenUsage.cache_read_input_tokens
      : 'totalCacheReadTokens' in tokenUsage
        ? tokenUsage.totalCacheReadTokens
        : undefined
  const output =
    'output_tokens' in tokenUsage
      ? tokenUsage.output_tokens
      : 'totalOutputTokens' in tokenUsage
