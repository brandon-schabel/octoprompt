...
export function MCPAnalyticsTabView({ projectId }: MCPAnalyticsTabViewProps) {
  ...
  const { data: overview, isLoading: overviewLoading, refetch: refetchOverview } = useGetMCPAnalyticsOverview(projectId, analyticsRequest)
  ...
  const topToolsData = (overviewData.topTools && overviewData.topTools.length > 0) 
    ? overviewData.topTools 
    : statistics?.data || []
    ...
    <div className="h-full flex flex-col p-4 md:p-6 overflow-hidden">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-4">
          ...
          <Badge variant="secondary">
            {overviewData.totalExecutions} executions
          </Badge>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {overviewData.uniqueTools} unique tools
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-2xl font-bold">
              {(overviewData.overallSuccessRate * 100).toFixed(1)}%
            </p>
            <p className="text-xs text-muted-foreground mt-1">
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-2xl font-bold">
              {(overviewData.avgExecutionTime / 1000).toFixed(2)}s
            </p>
            <p className="text-xs text-muted-foreground mt-1">
              Average duration
            </p>
        ...
        <TabsContent value="tools" className="flex-1 overflow-hidden mt-4">
          <Card className="h-full flex flex-col">
            <CardHeader>
              ...
              <CardDescription>
                Tool usage statistics for the selected time period
              </CardDescription>
            </CardHeader>
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Badge 
                            variant={tool.successRate > 0.9 ? "default" : tool.successRate > 0.7 ? "warning" : "destructive"}
                            className={cn(
        ...
        <TabsContent value="executions" className="flex-1 overflow-hidden mt-4">
          <Card className="h-full flex flex-col">
            <CardHeader>
              ...
              <CardDescription>
                Latest tool executions across the project
              </CardDescription>
            </CardHeader>
            <CardContent className="flex-1 min-h-0">
              <ScrollArea className="h-full">
                <div className="space-y-2">
                  {executions?.executions?.length > 0 ? executions.executions.map((execution: any) => (
                    <div 
                      key={execution.id}
                    ...
                    >
                        ...
                        <div>
                          <p className="font-medium">
                            ...
                            {getActionFromParams(execution.inputParams) && (
                              <span className="text-muted-foreground"> · {getActionFromParams(execution.inputParams)}</span>
                            )}
                          </p>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {execution.outputSize && (
                          <Badge variant="outline">
                            {(execution.outputSize / 1024).toFixed(1)} KB
                          </Badge>
                        )}
        ...
        <TabsContent value="timeline" className="flex-1 overflow-hidden mt-4">
          <Card className="h-full flex flex-col">
            <CardHeader>
              ...
              <CardDescription>
                Tool execution patterns over time
              </CardDescription>
            </CardHeader>
            <CardContent className="flex-1 min-h-0">
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p className="text-sm mt-1">
                  Track execution patterns and trends over time
                </p>
              </div>
            </CardContent>
          </Card>
                            ...
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <p className="font-medium">
                                ...
                                {getActionFromParams(error.inputParams) && (
                                  <span className="text-muted-foreground"> · {getActionFromParams(error.inputParams)}</span>
                                )}
                              </p>
