...
export function AgentCoderTabView({ project, projectId, allProjects }: AgentCoderTabViewProps) {
  ...
  const selectedFiles = useMemo(() => {
    ...
    return activeProjectTabState.selectedFiles
        ...
        <CardHeader className="flex-none pb-3">
          <div className="flex items-center justify-between">
            ...
            <Button
              ...
              title="Refresh Agent Coder with current context"
            >
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1">
                    {selectedFiles.map((file) => file && (
                      <div
                        key={file.id}
                        className="bg-muted/50 rounded-md px-2 py-1.5 text-xs font-mono"
                      >
                        {file.path}
                      </div>
                    ))}
      ...
      <div className="h-full flex flex-col">
        <div className="border-b">
          ...
          >
            <span>Context</span>
            <ChevronDown
              className={`h-4 w-4 transition-transform ${isContextOpen ? 'rotate-180' : ''}`}
            />
          </Button>
        </div>
        {isContextOpen && (
