...
export function CommitCard({ commit, onClick }: CommitCardProps) {
  ...
  const relativeTime =
    commit.relativeTime ||
    (dateIsValid ? formatDistanceToNow(new Date(commit.authoredDate), { addSuffix: true }) : 'Date unavailable')
  const fullDate = dateIsValid ? format(new Date(commit.authoredDate), 'PPpp') : 'Date unavailable'
                ...
                <div className='flex items-center gap-4 text-sm text-muted-foreground'>
                  ...
                  {commit.stats && (
                    <div className='flex items-center gap-1'>
                      ...
                      {commit.stats.additions > 0 && <span className='text-green-600'>+{commit.stats.additions}</span>}
                      {commit.stats.deletions > 0 && <span className='text-red-600'>-{commit.stats.deletions}</span>}
                    </div>
                  )}
                </div>
