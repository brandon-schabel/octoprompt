/**
 * @deprecated This component has been replaced by AgentCoderTabView.
...
export function AgentCoderControlDialog({
    ...
    const {
        ...
        error: dataError,
        refetch: refetchData
    } = useGetAgentCoderData(projectId, selectedJobId ?? -1)
    const runAgentCoderMutation = useRunAgentCoder()
    ...
    const {
        ...
        error: logError,
        refetch: refetchLogs
    } = useGetAgentCoderLogs(projectId, selectedJobId)

    const confirmChangesMutation = useConfirmAgentCoderChanges()
    ...
    const { data: listData, isLoading: isListLoading, refetch: refetchList } = useGetAgentCoderRuns(projectId)
