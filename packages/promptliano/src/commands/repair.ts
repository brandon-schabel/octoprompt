...
async function runDiagnostics(installPath: string): Promise<Issue[]> {
  ...
  const requiredDirs = ['packages/server', 'packages/client', 'packages/shared', 'packages/schemas']
  ...
  // Check executables
  const executables = ['node_modules/.bin/bun', 'packages/server/start-server.ts']
...
async function performRepairs(installPath: string, issues: Issue[], options: RepairOptions): Promise<RepairResult[]> {
