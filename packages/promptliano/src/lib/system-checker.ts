...
export class SystemChecker {
  ...
  async checkBun(): Promise<{
    ...
    try {
      ...
      const { stdout: pathOutput } = await safeExec(
        platform() === 'win32' ? 'where bun' : 'which bun'
      );
    ...
    for (const editor of editors) {
      if (existsSync(editor.path)) {
        try {
          ...
          if (config[mcpKey]) {
            const promptlianoServers = Object.keys(config[mcpKey])
              .filter(key => key.includes('promptliano'));
