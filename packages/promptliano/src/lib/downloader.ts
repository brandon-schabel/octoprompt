...
export class PromptlianoDownloader {
  ...
  async download(options: DownloadOptions): Promise<DownloadResult> {
    try {
      ...
      const asset = release.assets.find((a) => a.name.includes('bun-bundle') || a.name.includes('promptliano-bundle'))
      ...
      const checksumAsset = release.assets.find((a) => a.name.includes('checksums') || a.name.includes('sha256'))
  ...
  private async getLatestRelease(version?: string): Promise<GitHubRelease> {
    const url = version ? `${this.githubApiUrl}/tags/${version}` : `${this.githubApiUrl}/latest`
  ...
  private async extractBundle(bundlePath: string, targetPath: string): Promise<void> {
    ...
    if (bundlePath.endsWith('.zip')) {
      ...
      await pipeline(createReadStream(bundlePath), extract({ cwd: targetPath }))
    } else {
  ...
  private async verifyChecksum(filePath: string, checksumUrl: string, fileName: string): Promise<boolean> {
    try {
  ...
  private async downloadSource(tagName: string, options: DownloadOptions): Promise<DownloadResult> {
    try {
  ...
  private async extractSource(zipPath: string, targetPath: string, tagName: string): Promise<void> {
