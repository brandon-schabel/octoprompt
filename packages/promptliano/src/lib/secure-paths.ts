...
export class SecurePathValidator {
  ...
  constructor() {
    ...
    this.allowedPaths = [homedir(), process.cwd(), '/tmp', '/var/tmp']
  }
  ...
  validatePath(inputPath: string, basePath?: string): { valid: boolean; safePath?: string; error?: string } {
    try {
      ...
      const resolvedPath = basePath ? resolve(basePath, inputPath) : resolve(inputPath)
...
export class SafeFileOperations {
   ...
   */
  async validateOperation(operation: 'read' | 'write' | 'delete', path: string): Promise<void> {
    ...
    if (operation === 'delete') {
      ...
      const protectedPaths = [homedir(), '/', process.cwd()]
