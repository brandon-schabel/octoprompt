...
import { ProjectFileSchema, ProjectFileMapSchema } from 'shared/src/schemas/project.schemas';
...
export const AgentCoderRunDataSchema = z.object({
    updatedFiles: z.array(ProjectFileSchema).openapi({ description: 'List of files updated or created by the agent' }),
    taskPlan: AgentTaskPlanSchema.nullable().optional().openapi({ description: 'The final plan executed by the agent' }),
    agentJobId: z.string().openapi({ description: 'The unique ID for this agent run' }),
}).openapi('AgentCoderRunData');
