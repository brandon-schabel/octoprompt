...
export const AgentTaskSchema = z.object({
    ...
    dependencies: z.array(z.string().min(1)).optional().openapi({
...
}).openapi('AgentTask', { description: "Represents a single, well-defined unit of work required to fulfill part of the user's overall request, typically focused on one file." });


// Schema for the AI's file rewrite response
export const AgentFileRewriteResponseSchema = z.object({
    updatedContent: z.string().openapi({
        description: "The complete, updated content of the file after applying the changes requested in the task description.",
...
}).openapi('AgentFileRewriteResponse', { description: "The structured response expected from the file rewrite AI agent." });
...
export const AgentContextSchema = z.object({
    userInput: z.string().min(1).openapi({ description: "The original user request.", example: "Please implement JWT authentication." }),
    projectFiles: z.array(ProjectFileSchema).min(1, "At least one project file is required for context.").openapi({ description: "Array of project files provided as context." }),
    projectFileMap: ProjectFileMapSchema.refine(map => map.size > 0, { message: "Project file map cannot be empty." }).openapi({ description: "Map representation of project files for quick lookup." }),
...
export const AgentTaskPlanSchema = z.object({
    projectId: z.string().openapi({ description: "The ID of the project context in which these tasks operate.", example: "proj-abc-123" }),
    overallGoal: z.string().openapi({ description: "A concise summary of the original user request being addressed by this plan.", example: "Implement JWT-based authentication flow." }),
    tasks: z.array(AgentTaskSchema).min(1).openapi({ description: "An ordered list of tasks designed to collectively achieve the overall goal. Order implies execution sequence unless overridden by dependencies." }),
}).openapi('AgentTaskPlan', { description: "A structured execution plan containing an ordered list of tasks derived from the user request, project summary, and file context." });
...
export const AgentDataLogSchema = z.object({
    agentJobDirPath: z.string().openapi({ description: "Absolute path to the directory containing logs for this job." }),
    ...
    taskPlan: AgentTaskPlanSchema.optional().openapi({ description: "The initial task plan generated by the planning agent (before execution)." }),
    // Final status after the orchestrator attempted to run the plan
    finalStatus: z.enum(['Success', 'Failed', 'No tasks generated', 'Error']).openapi({ description: "The final outcome status of the agent run." }),
    // The plan state after execution attempt (tasks have final statuses)
    finalTaskPlan: AgentTaskPlanSchema.nullable().openapi({ description: "The task plan reflecting the state after execution attempts (tasks will have final statuses like COMPLETED, FAILED)." }),
    agentJobEndTime: z.string().datetime().openapi({ description: "ISO 8601 timestamp when the agent job finished or errored." }),
    ...
    updatedFiles: z.array(ProjectFileSchema).optional().openapi({ description: "List of files with proposed changes (new files or modified files with different checksums)." }),
