...
export async function executeSimpleAgentOrchestrator(input: OrchestratorInput): Promise<OrchestratorOutput> {
  try {
    const { projectId, userRequest, selectedFileIds, agentJobId } = input
    ...
    const summaryText = typeof projectSummary === 'string' ? projectSummary : projectSummary.message || 'No summary available'
...
**Project Summary:** ${summaryText.substring(0, 500)}...
...
Keep the response concise and actionable.`
    ...
    const planningResponse = await enhancedPlanningAgent.generate([
...
Project context: ${summaryText.substring(0, 300)}...
...
Please provide the complete updated file content. Make sure the code is functional and follows best practices.`
      ...
      const codingResponse = await enhancedCoderAgent.generate([
