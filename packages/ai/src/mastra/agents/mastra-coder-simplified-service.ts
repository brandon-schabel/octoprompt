...
import { analyzeCodeTool } from '../tools/file-tools'
import { executeSimpleAgentOrchestrator } from '../workflows/simple-agent-orchestrator'
...
export async function executeMastraCodeChange(request: MastraCoderRequest): Promise<MastraCoderResult> {
  ...
  try {
    ...
    const workflowResult = await executeSimpleAgentOrchestrator({
      projectId,
      userRequest,
      selectedFileIds,
      agentJobId
...
export async function buildSimpleProjectContext(
  ...
  // const { getFullProjectSummary } = await import('@octoprompt/shared')
