  ...
  execute: async ({ context }) => {
    try {
      ...
      if (existingFile) {
        ...
        const { bulkCreateProjectFiles } = await import('@/services/project-service');
  ...
  execute: async ({ context }) => {
    try {
      ...
      if (analysisType === 'structure' || analysisType === 'all') {
        const functions = [...content.matchAll(/(?:function|const\s+\w+\s*=\s*(?:async\s+)?(?:function|\([^)]*\)\s*=>))/g)]
          .map(match => {
      ...
      if (analysisType === 'dependencies' || analysisType === 'all') {
        const imports = [...content.matchAll(/import\s+[^;]+from\s+['"`]([^'"`]+)['"`]/g)]
          .map(match => match[1]);
        const exports = [...content.matchAll(/export\s+(?:default\s+)?(?:function|class|interface|type|const|let|var)\s+(\w+)/g)]
          .map(match => match[1]);
        const externalDeps = imports.filter(imp => !imp.startsWith('.') && !imp.startsWith('/'));
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          switch (searchType) {
            ...
            case 'regex':
              ...
              break;
            case 'function':
              isMatch = line.includes(`function ${query}`) || line.includes(`${query} =`) || line.includes(`const ${query}`);
              break;
            case 'class':
