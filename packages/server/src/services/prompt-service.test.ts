...
describe('Prompt Service', () => {
  ...
  beforeEach(async () => {
    ...
    db.run(`INSERT INTO projects (name, path) VALUES (?, ?)`, [
      `TestProject_${randomString()}`,
      `/test/path_${randomString()}`
    ])
  ...
  test('createPrompt inserts a new prompt', async () => {
    ...
    // Check DB
    const row = db.query('SELECT * FROM prompts WHERE id = ?').get(created.id)
  ...
  test('addPromptToProject then removePromptFromProject associates/unassociates prompt', async () => {
    ...
    db.run(`INSERT INTO projects (name, path) VALUES (?, ?)`, ['ProjForPrompt', '/fake/path'])
    const projectId = db.query('SELECT last_insert_rowid() as id').get().id

    // Associate
    await addPromptToProject(prompt.id, projectId)

    const rows = db.query('SELECT * FROM prompt_projects WHERE prompt_id = ?').all(prompt.id)
    expect(rows.length).toBe(1)
    expect(Number(rows[0].project_id)).toBe(projectId)

    // Remove association
    await removePromptFromProject(prompt.id, projectId)
    const afterRemoval = db.query('SELECT * FROM prompt_projects WHERE prompt_id = ?').all(prompt.id)
