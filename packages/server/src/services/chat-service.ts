...
function mapDbRowToChat(row: RawChat): Chat {
  // Expects a valid RawChat object
  const mapped = {
    id: row.id,
    title: row.title,
  ...
  if (mapped.createdAt === null || mapped.updatedAt === null) {
    ...
    throw new ApiError(500, `Data integrity issue: Failed to parse date for chat ${row.id}.`, 'CHAT_DATE_CORRUPT', {
      id: row.id,
  ...
  if (!result.success) {
    console.error(`Failed to parse chat data for ID ${row.id}: ${result.error.message}`, {
      raw: row,
      mapped,
    ...
    throw new ApiError(
      500,
      `Data integrity issue: Failed to parse chat data for ID ${row.id}.`,
      'CHAT_DATA_CORRUPT',
...
function mapDbRowToChatMessage(row: RawChatMessage): ChatMessage {
  // Expects a valid RawChatMessage object
  const mapped = {
    id: row.id,
    chatId: row.chat_id,
    role: row.role,
  ...
  if (mapped.createdAt === null) {
    ...
    throw new ApiError(
      500,
      `Data integrity issue: Failed to parse date for message ${row.id}.`,
      'MESSAGE_DATE_CORRUPT',
      { id: row.id, createdAtRaw: row.created_at }
    )
  }

  const result = ChatMessageSchema.safeParse(mapped)
  if (!result.success) {
    console.error(`Failed to parse chat message data for ID ${row.id}: ${result.error.message}`, {
      raw: row,
      mapped,
      errors: result.error.flatten().fieldErrors
    })
    throw new ApiError(
      500,
      `Data integrity issue: Failed to parse chat message data for ID ${row.id}.`,
      'MESSAGE_DATA_CORRUPT',
      result.error.flatten().fieldErrors
    )
  }
  return result.data
...
export function createChatService() {
  async function createChat(title: string, options?: CreateChatOptions): Promise<Chat> {
    if (options?.copyExisting && options?.currentChatId) {
      const sourceChatExists = db.prepare('SELECT id FROM chats WHERE id = ?').get(options.currentChatId) as
        | { id: string }
        | undefined
      if (!sourceChatExists) {
        throw new ApiError(
  ...
  async function saveMessage(message: ExtendedChatMessage): Promise<ExtendedChatMessage> {
    ...
    if (!rawSaved) {
      throw new ApiError(
        500,
        'Failed to save chat message: database did not return message data.',
        'MESSAGE_SAVE_FAILED'
      )
    }
  ...
  async function forkChat(sourceChatId: string, excludedMessageIds: string[] = []): Promise<Chat> {
    ...
    if (!rawNewChat) {
      throw new ApiError(
        500,
        'Failed to create forked chat: database did not return new chat data.',
        'CHAT_FORK_CREATION_FAILED'
      )
    }
  ...
  async function forkChatFromMessage(
    ...
    if (rawStartMessage.chat_id !== sourceChatId) {
      throw new ApiError(
        400,
        `Message ${messageId} does not belong to the specified chat ${sourceChatId}.`,
    ...
    if (!rawNewChat) {
      throw new ApiError(
        500,
        'Failed to create forked chat from message: database did not return new chat data.',
        'CHAT_FORK_MESSAGE_CREATION_FAILED'
      )
    }
    const newChat = mapDbRowToChat(rawNewChat)
    ...
    const messagesToCopy = sourceMessages.slice(0, indexOfStart + 1).filter((m) => !excludedMessageIds.includes(m.id))
