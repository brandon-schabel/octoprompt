...
export function createFileChangeWatcher() {
    ...
    function startWatching(options: WatchOptions): void {
        ...
        if (watcherInstance) { // Stop previous watcher if directory changes
            stopWatching();
        }

        if (!nodeFsExistsSync(resolvedDir)) { // Use nodeFsExistsSync for general FS checks
            console.warn(`[FileChangeWatcher] Directory does not exist, cannot watch: ${resolvedDir}`);
            return;
        }

        try {
            watcherInstance = fsWatch(
                resolvedDir,
                { recursive },
                (eventType, filename) => {
...
const CRITICAL_EXCLUDED_DIRS = new Set(['node_modules', '.git', 'dist', 'build', '.vscode', '.idea', 'venv', '.DS_Store']);
...
export async function syncFileSet(
    ...
    for (const absFilePath of absoluteFilePathsOnDisk) {
        ...
        try {
            ...
            console.error(`[FileSync] Error processing file ${absFilePath} (relative: ${normalizedRelativePath}): ${fileError.message}. Skipping.`);
...
export async function syncProject(project: Project): Promise<{ created: number; updated: number; deleted: number; skipped: number }> {
    try {
        ...
        const projectFilesOnDisk = getTextFiles(absoluteProjectPath, absoluteProjectPath, ignoreFilter, ALLOWED_FILE_CONFIGS);
...
export async function syncProjectFolder(
    ...
    try {
        ...
        const folderFilesOnDisk = getTextFiles(absoluteFolderToSync, absoluteProjectPath, ignoreFilter, ALLOWED_FILE_CONFIGS);
...
export function createWatchersManager() {
    ...
    async function startWatchingProject(project: Project, ignorePatterns: string[] = []): Promise<void> {
        ...
        if (!nodeFsExistsSync(resolvedProjectPath)) { // Use nodeFsExistsSync
