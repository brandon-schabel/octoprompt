...
export async function parseTestResults(xmlPath: string): Promise<ParsedJUnitResults> {
    ...
    try {
                    ...
                    for (const testcase of cases) {
                        if (testcase.failure) {
                            failures.push({
                                ...
                                message: typeof testcase.failure === 'string' ? testcase.failure : testcase.failure['@_message'] ?? 'No message',
        ...
        return { passed: false, totalTests: 0, failedTests: 0, failures: [{ testName: "Parsing Error", message: `Could not parse ${xmlPath}` }] };
    }
} 