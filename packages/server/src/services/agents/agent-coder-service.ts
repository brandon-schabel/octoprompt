...
export async function mainOrchestrator(rawAgentContext: CoderAgentDataContext): Promise<{ success: boolean; files: ProjectFile[]; tasks: TaskPlan | null; agentJobId: string; agentDataLog: Record<string, any> }> {
    ...
    // agentDataLog.projectSummaryContext = rawAgentContext.projectSummaryContext
    ...
    try {
        ...
        if (!finalTaskPlan || finalTaskPlan.tasks.length === 0) {
            ...
            agentDataLog.finalFiles = []; // No files changed
            await writeAgentDataLog(agentJobId, agentDataLog); // Write data log before exiting
        ...
        agentDataLog.finalFiles = filesFromTaskPlan.map(f => ({ id: f.id, path: f.path, checksum: f.checksum })); // Log file summary
        agentDataLog.finalTaskPlan = tasksFromTaskPlan; // Log final task state
        agentDataLog.agentJobEndTime = new Date().toISOString();

        await writeAgentDataLog(agentJobId, agentDataLog); // Write final data log
        ...
        agentDataLog.agentJobEndTime = new Date().toISOString();
        agentDataLog.finalFiles = filesFromTaskPlan.map(f => ({ id: f.id, path: f.path, checksum: f.checksum })); // Log files even on error
        agentDataLog.finalTaskPlan = tasksFromTaskPlan; // Log tasks even on error

        await writeAgentDataLog(agentJobId, agentDataLog); // Write data log on error
