...
const ERROR_RECOVERY_SUGGESTIONS: Record<MCPErrorCode, string> = {
  ...
  [MCPErrorCode.PROJECT_NOT_FOUND]: 'Verify the project ID exists. Use the project_manager list action to see available projects.',
  [MCPErrorCode.FILE_NOT_FOUND]: 'Check the file path is correct. Use browse_files to explore available files.',
  [MCPErrorCode.TICKET_NOT_FOUND]: 'Verify the ticket ID exists. Use ticket_manager list action to see available tickets.',
  [MCPErrorCode.PROMPT_NOT_FOUND]: 'Check the prompt ID exists. Use prompt_manager list action to see available prompts.',
  [MCPErrorCode.RESOURCE_ALREADY_EXISTS]: 'The resource already exists. Use update action instead of create, or choose a different name.',
  ...
  [MCPErrorCode.OPERATION_FAILED]: 'The operation could not be completed. Check the error details for more information.',
...
export class MCPError extends ApiError {
  ...
  constructor(
    code: MCPErrorCode,
    message: string,
    context?: MCPErrorDetails['context'],
