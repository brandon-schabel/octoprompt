...
import { AgentCoderRunRequestSchema, AgentCoderRunResponseSchema, CoderAgentDataContext, AgentDataLogSchema } from 'shared/src/schemas/agent-coder.schemas';
import { AGENT_LOGS_DIR, getOrchestratorLogFilePaths, listAgentJobs, getAgentDataLogFilePath } from '@/services/agents/agent-logger';
    ...
    for (const line of lines) {
        ...
        if (trimmedLine) { // Skip empty lines
            try {
                ...
                await log(`[JSONL Parser] Skipping invalid line: ${trimmedLine.substring(0, 100)}...`, 'warn', { error: error.message });
            }
        }
    }
    ...
    try {
        const writePromises = updatedFiles.map(async (updatedFile) => {
            if (updatedFile.content === null || updatedFile.content === undefined) {
                await log(`[Agent Coder Route ${agentJobId}] Skipping write for ${updatedFile.path} (null content).`, 'warn', { agentJobId, filePath: updatedFile.path });
                return;
            }
            ...
            try {
                ...
                await log(`[Agent Coder Route ${agentJobId}] Wrote code file: ${updatedFile.path}`, 'info', { agentJobId, filePath: updatedFile.path });
            } catch (writeError: any) {
                await log(`[Agent Coder Route ${agentJobId}] Failed to write ${updatedFile.path}: ${writeError.message}`, 'error', { agentJobId, filePath: updatedFile.path, error: writeError.message, stack: writeError.stack });
        ...
        if (failedWrites.length > 0) {
            await log(`[Agent Coder Route ${agentJobId}] ${failedWrites.length} file write(s) failed:`, 'error', { agentJobId, failedCount: failedWrites.length });
            failedWrites.forEach(async (f) => await log(`- ${(f as PromiseRejectedResult).reason}`, 'error', { agentJobId, reason: (f as PromiseRejectedResult).reason }));
        ...
        await log(`[Agent Coder Route ${agentJobId}] Error setting up code file writing: ${fileWriteSetupError.message}`, 'error', { agentJobId, error: fileWriteSetupError.message, stack: fileWriteSetupError.stack });
    }
}
...
export const agentCoderRoutes = new OpenAPIHono()
    .openapi(runAgentCoderRoute, async (c) => {
        ...
        if (!project) {
            await log(`[Agent Coder Route ${routeAgentJobId}] Project ${routeProjectId} not found.`, 'error', { agentJobId: routeAgentJobId, projectId: routeProjectId });
        ...
        if (typeof projectSummaryContext !== 'string') {
            await log(`[Agent Coder Route ${routeAgentJobId}] Project summary context error for project ${routeProjectId}.`, 'error', { agentJobId: routeAgentJobId, projectId: routeProjectId });
        ...
        if (projectFiles.filter(f => routeSelectedFileIds.includes(f.id)).length === 0 && routeSelectedFileIds.length > 0) {
            await log(`[Agent Coder Route ${routeAgentJobId}] No matching files found for IDs: ${routeSelectedFileIds.join(', ')}`, 'warn', { agentJobId: routeAgentJobId, selectedFileIds: routeSelectedFileIds });
        }

        await log(`[Agent Coder Route ${routeAgentJobId}] Calling main orchestrator...`, 'info', { agentJobId: routeAgentJobId });
        ...
        await log(`[Agent Coder Route ${routeAgentJobId}] Orchestrator finished successfully.`, 'info', { agentJobId: routeAgentJobId });
    ...
    .openapi(confirmAgentRunChangesRoute, async (c) => {
        ...
        const dataFile = Bun.file(dataFilePath);
        if (!(await dataFile.exists())) {
            await log(`[Agent Confirm Route] Data file not found at path: ${dataFilePath}`, 'error', { agentJobId, dataFilePath });
        ...
        if (!validationResult.success) {
            ...
            await log(`[Agent Confirm Route] Invalid data log structure for ${agentJobId}: ${error.message}`, 'error', { agentJobId, errorMessage: error.message, validationError: validationResult.error.format() });
        ...
        await log(`[Agent Confirm Route] Found ${agentProposedFiles.length} proposed file changes in data log for project ${projectId}.`, 'info', { agentJobId, projectId, fileCount: agentProposedFiles.length });

        const projectData = await getProjectById(projectId);
        if (!projectData) {
            await log(`[Agent Confirm Route ${agentJobId}] Project ${projectId} not found.`, 'error', { agentJobId, projectId });
        ...
        if (!absoluteProjectPath) {
            await log(`[Agent Confirm Route ${agentJobId}] Could not determine absolute path for project ${projectId}.`, 'error', { agentJobId, projectId });
            throw new ApiError(500, `Could not determine absolute path for project ${projectId}.`, 'PROJECT_PATH_RESOLUTION_FAILED');
        }
        await log(`[Agent Confirm Route] Absolute project path: ${absoluteProjectPath}`, 'info', { agentJobId, projectId, absoluteProjectPath });

        const originalProjectFiles = await getProjectFiles(projectId);
        if (!originalProjectFiles) {
            await log(`[Agent Confirm Route ${agentJobId}] Original files for project ${projectId} could not be fetched or project is empty.`, 'error', { agentJobId, projectId });
        ...
        await log(`[Agent Confirm Route] Built original project file map with ${originalProjectFileMap.size} files.`, 'info', { agentJobId, projectId, mapSize: originalProjectFileMap.size });
    ...
    .openapi(deleteAgentRunRoute, async (c) => {
        ...
        try {
            ...
            if (checkError.code === 'ENOENT') {
                await log(`[Agent Delete Route] Directory not found for ${agentJobId}: ${agentRunDirectory}`, 'warn', { agentJobId, directory: agentRunDirectory, errorCode: checkError.code });
                throw new ApiError(404, `Agent run ${agentJobId} not found. Directory does not exist.`, 'AGENT_RUN_DIR_NOT_FOUND');
            }
            await log(`[Agent Delete Route] Error checking directory for ${agentJobId}: ${checkError.message}`, 'error', { agentJobId, directory: agentRunDirectory, error: checkError.message, stack: checkError.stack });
            throw new ApiError(500, `Error checking agent run directory ${agentJobId}: ${checkError.message}`, 'AGENT_RUN_DIR_STAT_FAILED');
        }

        await log(`[Agent Delete Route] Attempting to delete directory: ${agentRunDirectory}`, 'info', { agentJobId, directory: agentRunDirectory });
        await rm(agentRunDirectory, { recursive: true, force: true }); // rm can throw
        await log(`[Agent Delete Route] Successfully deleted directory for ${agentJobId}`, 'info', { agentJobId, directory: agentRunDirectory });
