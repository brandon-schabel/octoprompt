...
import {
  ...
  ListAgentCoderRunsResponseSchema,
  GetAgentCoderLogsResponseSchema,
  GetAgentCoderDataResponseSchema,
  ConfirmAgentCoderChangesResponseSchema,
  DeleteAgentCoderRunResponseSchema
...
import { ApiError, buildProjectFileMap } from '@octoprompt/shared'
import { fromZodError } from 'zod-validation-error'
// import { log, getOrchestratorLogFilePaths, getAgentDataLogFilePath, AGENT_LOGS_DIR } from '../../../services/src/agents/agent-logger'
import { agentLogger } from '@octoprompt/services'

const { log, getOrchestratorLogFilePaths, getAgentDataLogFilePath, AGENT_LOGS_DIR } = agentLogger

// Placeholder for mainOrchestrator function - should be implemented in agent services
async function mainOrchestrator(
  context: CoderAgentDataContext
): Promise<{ updatedFiles: number[]; taskPlan: any | null; agentJobId: number }> {
  // TODO: Implement actual orchestrator logic
  console.log('mainOrchestrator called with context:', context.agentJobId)
  return {
    updatedFiles: [],
    taskPlan: null,
...
// Function to list agent jobs for a project
async function listAgentJobs(projectId: number): Promise<number[]> {
  try {
    const projectJobsDir = join(AGENT_LOGS_DIR, 'projects', projectId.toString(), 'jobs')
    const entries = await readdir(projectJobsDir, { withFileTypes: true })
...
const getAgentRunLogsRoute = createRoute({
  ...
  responses: {
    200: {
      content: { 'application/json': { schema: GetAgentCoderLogsResponseSchema } },
...
const getAgentRunDataRoute = createRoute({
  ...
  responses: {
    200: {
      content: { 'application/json': { schema: GetAgentCoderDataResponseSchema } },
...
const listAgentRunsRoute = createRoute({
  ...
  responses: {
    200: {
      content: { 'application/json': { schema: ListAgentCoderRunsResponseSchema } },
...
const confirmAgentRunChangesRoute = createRoute({
  ...
  responses: {
    200: {
      content: { 'application/json': { schema: ConfirmAgentCoderChangesResponseSchema } },
...
// Schema for delete route - only needs agentJobId
const DeleteAgentJobIdParamsSchema = z.object({
  agentJobId: z.number().openapi({ description: 'The unique ID of the agent run to delete.' })
})
...
const deleteAgentRunRoute = createRoute({
  ...
  request: {
    params: DeleteAgentJobIdParamsSchema
  },
  responses: {
    200: {
      content: { 'application/json': { schema: DeleteAgentCoderRunResponseSchema } },
...
export const agentCoderRoutes = new OpenAPIHono()
  .openapi(runAgentCoderRoute, async (c) => {
    ...
    const prompts = await getPromptsByIds(selectedPromptIds?.filter((id): id is number => id !== undefined) || [])
  ...
  .openapi(listAgentRunsRoute, async (c) => {
    ...
    const responsePayload: z.infer<typeof ListAgentCoderRunsResponseSchema> = {
      success: true,
      data: jobIds
    }
    return c.json(responsePayload, 200)
  ...
  .openapi(getAgentRunLogsRoute, async (c) => {
    ...
    const responsePayload: z.infer<typeof GetAgentCoderLogsResponseSchema> = {
      success: true,
      data: parsedLogs
    }
    return c.json(responsePayload, 200)
  ...
  .openapi(getAgentRunDataRoute, async (c) => {
    ...
    const responsePayload: z.infer<typeof GetAgentCoderDataResponseSchema> = {
      success: true,
      data: agentData
    }
    return c.json(responsePayload, 200)
  ...
  .openapi(confirmAgentRunChangesRoute, async (c) => {
      ...
      return c.json(
        {
          ...
          writtenFiles: []
        } satisfies z.infer<typeof ConfirmAgentCoderChangesResponseSchema>,
        200
      )
    }
    ...
    const successPayload: z.infer<typeof ConfirmAgentCoderChangesResponseSchema> = {
      success: true,
  ...
  .openapi(deleteAgentRunRoute, async (c) => {
    ...
    const successPayload: z.infer<typeof DeleteAgentCoderRunResponseSchema> = {
      success: true,
