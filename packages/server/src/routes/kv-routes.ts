...
export const kvRoutes = new OpenAPIHono()
    ...
    .patch('/api/kv/:key', async (c) => {
        ...
        if (!body || typeof body.value === 'undefined') {
            throw new ApiError(400, "Request body must include a 'value' property containing the partial update.", "INVALID_REQUEST_BODY_PATCH");
        }
        ...
        try {
            ...
            if (!(schemaToParse instanceof z.ZodObject)) {
                console.error(`[KV Route - PATCH] Schema for key '${key}' is not a ZodObject and cannot be partially validated directly. Schema type: ${schemaToParse._def.typeName}`);
                ...
                // Consider throwing new ApiError(400, `Partial updates are not supported for the schema type of key '${key}'.`, 'KV_PARTIAL_NOT_SUPPORTED');
                 ...
                 throw new ApiError(400, `Partial updates are only supported for object-based KV stores. Key '${key}' has a different schema type.`, 'KV_PARTIAL_OBJECT_ONLY');
            }
            ...
            if (error instanceof ZodError) {
                throw new ApiError(400, `Validation Error: Partial value for key '${key}' is invalid. Issues: ${error.errors.map(e => e.message).join(', ')}`, 'KV_INVALID_PARTIAL_VALUE', { issues: error.errors });
            }
            throw new ApiError(400, `Validation Error: Partial value for key '${key}' is invalid.`, 'KV_INVALID_PARTIAL_VALUE');
        }
