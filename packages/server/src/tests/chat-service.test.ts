...
describe("Chat Service", () => {
    beforeEach(async () => {
        // We create a fresh instance of the service each time
        // so it references the newly created in-memory DB
    ...
    test("getAllChats returns all chats sorted by updatedAt", async () => {
        ...
        // They are sorted by updatedAt ascending or descending (depending on your code),
        // you could check or just ensure it includes them
    });
    ...
    test("deleteMessage removes only that message", async () => {
        ...
        const m1 = await chatService.saveMessage({ chatId: chat.id, role: "user", content: "First" });
        const m2 = await chatService.saveMessage({ chatId: chat.id, role: "assistant", content: "Second" });
    ...
    test("forkChatFromMessage only copies messages up to a given message, excluding any if needed", async () => {
        ...
        const msg1 = await chatService.saveMessage({ chatId: source.id, role: "user", content: "Msg1" });
        const msg2 = await chatService.saveMessage({ chatId: source.id, role: "assistant", content: "Msg2" });
        const msg3 = await chatService.saveMessage({ chatId: source.id, role: "user", content: "Msg3" });
